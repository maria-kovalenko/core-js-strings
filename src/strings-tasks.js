...
function getStringLength(value) {
  return !!value === false ? 0 : value.length;
}
...
function isPalindrome(str) {
  ...
  const arr = [...trim.toLowerCase()];
  const reverseArr = [];
  for (let i = 0; i < arr.length; i += 1) {
    reverseArr.unshift(arr[i].toLowerCase());
  }
...
function findLongestWord(sentence) {
  ...
  const bigWord = arr.map((item) => item.length);
  const max = Math.max(...bigWord);
  const res = [];
  for (let i = 0; i < arr.length; i += 1) {
    if (arr[i].length === max) {
      res.push(arr[i]);
...
function reverseWords(str) {
  ...
  const res = arrayFromStr.map((item) => [...item].reverse().join(''));
  return res.join(' ');
}
